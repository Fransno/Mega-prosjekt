import rclpy
from rclpy.node import Node
from sensor_msgs.msg import Image
from std_msgs.msg import Float64MultiArray
from cv_bridge import CvBridge
import cv2
import numpy as np # Viktig for HSV-grenser

# (Senere: Importer din egendefinerte meldingstype for deteksjonsresultater)
# from your_custom_msgs.msg import CubeDetection, CubeDetections

class ColorDetectorNode(Node):
    def __init__(self):
        super().__init__('color_detector')

        # === Parametere for HSV-verdier (Veldig viktig!) ===
        # Definer standardverdier, men disse bør lastes fra en YAML-fil via launch-filen.
        # Eksempel for RØD farge:
        self.declare_parameter('red_hsv_lower', [0, 120, 70])
        self.declare_parameter('red_hsv_upper', [10, 255, 255])
        # Husk at rød farge ofte krysser 0/180 i Hue-skalaen, så du kan trenge to områder for rød.
        self.declare_parameter('red_hsv_lower2', [170, 120, 70])
        self.declare_parameter('red_hsv_upper2', [180, 255, 255])

        # Eksempel for GUL farge:
        self.declare_parameter('yellow_hsv_lower', [20, 100, 100])
        self.declare_parameter('yellow_hsv_upper', [30, 255, 255])

        # Eksempel for BLÅ farge:
        self.declare_parameter('blue_hsv_lower', [90, 100, 100]) # Juster disse verdiene!
        self.declare_parameter('blue_hsv_upper', [130, 255, 255]) # Juster disse verdiene!

        # Eksempel for GRØNN farge:
        self.declare_parameter('green_hsv_lower', [35, 100, 100])
        self.declare_parameter('green_hsv_upper', [85, 255, 255])

        # Parametere for bildebehandling
        self.declare_parameter('contour_min_area', 50) # Minimum areal for en gyldig kontur

        # Hent parameterverdier
        self.red_lower = np.array(self.get_parameter('red_hsv_lower').value, dtype=np.uint8)
        self.red_upper = np.array(self.get_parameter('red_hsv_upper').value, dtype=np.uint8)
        # self.red_lower2 = np.array(self.get_parameter('red_hsv_lower2').value, dtype=np.uint8)
        # self.red_upper2 = np.array(self.get_parameter('red_hsv_upper2').value, dtype=np.uint8)
        self.yellow_lower = np.array(self.get_parameter('yellow_hsv_lower').value, dtype=np.uint8)
        self.yellow_upper = np.array(self.get_parameter('yellow_hsv_upper').value, dtype=np.uint8)
        self.blue_lower = np.array(self.get_parameter('blue_hsv_lower').value, dtype=np.uint8)
        self.blue_upper = np.array(self.get_parameter('blue_hsv_upper').value, dtype=np.uint8)
        self.green_lower = np.array(self.get_parameter('green_hsv_lower').value, dtype=np.uint8)
        self.green_upper = np.array(self.get_parameter('green_hsv_upper').value, dtype=np.uint8)
        self.contour_min_area_ = self.get_parameter('contour_min_area').value


        # === Abonnement ===
        # Abonner på råbildet eller et allerede prosessert bilde (f.eks. blurred)
        self.subscription = self.create_subscription(
            Image,
            'image_raw',  # Eller '/image_blurred' hvis du vil bruke den først
            self.image_callback,
            10)

        # === Publisher(e) ===
        # 1. For feilsøking: Publiser masker eller bilder med konturer
        self.mask_publisher_red = self.create_publisher(Image, 'image_mask_red', 10)
        self.mask_publisher_yellow = self.create_publisher(Image, 'image_mask_yellow', 10)
        self.mask_publisher_blue = self.create_publisher(Image, 'image_mask_blue', 10)
        self.mask_publisher_green = self.create_publisher(Image, 'image_mask_green', 10)
        self.annotated_image_publisher = self.create_publisher(Image, 'image_annotated_detections', 10)

        # 2. For faktiske deteksjonsresultater (VIKTIG)
        # (Dette krever at du definerer CubeDetection.msg og CubeDetections.msg)
        self.detections_publisher = self.create_publisher(Float64MultiArray, 'detected_cubes', 10)

        self.bridge = CvBridge()
        self.get_logger().info('Color Detector Node Initialized')

    def image_callback(self, msg):
        try:
            cv_image = self.bridge.imgmsg_to_cv2(msg, "bgr8")
        except Exception as e:
            self.get_logger().error(f"Failed to convert image: {e}")
            return

        # Lag en kopi for annotering (valgfritt, for publisering av feilsøkingsbilde)
        annotated_image = cv_image.copy()

        # Konverter til HSV
        hsv_image = cv2.cvtColor(cv_image, cv2.COLOR_BGR2HSV)

        # Hovedarray for å samle koordinater
        main_array = []

        # Variabel for å samle total antall kuber
        total_count = 0

        # Bygge opp detected_cubes
        detections_msg = Float64MultiArray()
        color_codes = {
            "red": 1.0,
            "yellow": 2.0,
            "blue": 3.0,
            "green": 4.0
        }
        data = []

        # --- Prosessering for RØD ---
        mask_r = cv2.inRange(hsv_image, self.red_lower, self.red_upper)
        contours_r, _ = cv2.findContours(mask_r, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        red_count = 0
        red_coords = []
        for contour in contours_r:
            area = cv2.contourArea(contour)
            if area > self.contour_min_area_:
                red_count += 1
                total_count += 1
                M = cv2.moments(contour)
                if M["m00"] != 0:
                    cX = int(M["m10"] / M["m00"])
                    cY = int(M["m01"] / M["m00"])
                    red_coords.append((cX, cY))
                    cv2.drawContours(annotated_image, [contour], -1, (0, 0, 255), 2)
                    cv2.circle(annotated_image, (cX, cY), 5, (0, 0, 255), -1)
        main_array.append({"color": "red", "coordinates": red_coords})
        self.get_logger().info(f"Red cubes detected: {red_count}")

        # --- Prosessering for GUL ---
        mask_y = cv2.inRange(hsv_image, self.yellow_lower, self.yellow_upper)
        contours_y, _ = cv2.findContours(mask_y, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        yellow_count = 0
        yellow_coords = []
        for contour in contours_y:
            area = cv2.contourArea(contour)
            if area > self.contour_min_area_:
                yellow_count += 1
                total_count += 1
                M = cv2.moments(contour)
                if M["m00"] != 0:
                    cX = int(M["m10"] / M["m00"])
                    cY = int(M["m01"] / M["m00"])
                    yellow_coords.append((cX, cY))
                    cv2.drawContours(annotated_image, [contour], -1, (0, 255, 255), 2)
                    cv2.circle(annotated_image, (cX, cY), 5, (0, 255, 255), -1)
        main_array.append({"color": "yellow", "coordinates": yellow_coords})
        self.get_logger().info(f"Yellow cubes detected: {yellow_count}")

        # --- Prosessering for BLÅ ---
        mask_b = cv2.inRange(hsv_image, self.blue_lower, self.blue_upper)
        contours_b, _ = cv2.findContours(mask_b, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        blue_count = 0
        blue_coords = []
        for contour in contours_b:
            area = cv2.contourArea(contour)
            if area > self.contour_min_area_:
                blue_count += 1
                total_count += 1
                M = cv2.moments(contour)
                if M["m00"] != 0:
                    cX = int(M["m10"] / M["m00"])
                    cY = int(M["m01"] / M["m00"])
                    blue_coords.append((cX, cY))
                    cv2.drawContours(annotated_image, [contour], -1, (255, 0, 0), 2)
                    cv2.circle(annotated_image, (cX, cY), 5, (255, 0, 0), -1)
        main_array.append({"color": "blue", "coordinates": blue_coords})
        self.get_logger().info(f"Blue cubes detected: {blue_count}")

        # --- Prosessering for GRØNN ---
        mask_g = cv2.inRange(hsv_image, self.green_lower, self.green_upper)
        contours_g, _ = cv2.findContours(mask_g, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        green_count = 0
        green_coords = []
        for contour in contours_g:
            area = cv2.contourArea(contour)
            if area > self.contour_min_area_:
                green_count += 1
                total_count += 1
                M = cv2.moments(contour)
                if M["m00"] != 0:
                    cX = int(M["m10"] / M["m00"])
                    cY = int(M["m01"] / M["m00"])
                    green_coords.append((cX, cY))
                    cv2.drawContours(annotated_image, [contour], -1, (0, 255, 0), 2)
                    cv2.circle(annotated_image, (cX, cY), 5, (0, 255, 0), -1)
        main_array.append({"color": "green", "coordinates": green_coords})
        self.get_logger().info(f"Green cubes detected: {green_count}")

        # Log the arrays
        self.get_logger().info(f"Main Array (Coordinates): {main_array}")
        self.get_logger().info(f"Total Count of Cubes: {total_count}")

        # fyller data med fargekoder og koordinater
        for color_dict in main_array:
            color = color_dict["color"]
            for (x, y) in color_dict["coordinates"]:
                data.extend([color_codes[color], float(x), float(y)])
        detections_msg.data = data
        self.detections_publisher.publish(detections_msg)
        
        # Logg meldingen for feilsøking
        self.get_logger().info(f"Published detections: {detections_msg.data}")

        # === Publiser resultater ===
        try:
            # Publiser masker (for feilsøking)
            self.mask_publisher_red.publish(self.bridge.cv2_to_imgmsg(mask_r, "mono8"))
            self.mask_publisher_yellow.publish(self.bridge.cv2_to_imgmsg(mask_y, "mono8"))
            self.mask_publisher_blue.publish(self.bridge.cv2_to_imgmsg(mask_b, "mono8"))
            self.mask_publisher_green.publish(self.bridge.cv2_to_imgmsg(mask_g, "mono8"))
            self.annotated_image_publisher.publish(self.bridge.cv2_to_imgmsg(annotated_image, "bgr8"))
        except Exception as e:
            self.get_logger().error(f"Failed to publish images/detections: {e}")


def main(args=None):
    rclpy.init(args=args)
    node = ColorDetectorNode()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()

if __name__ == '__main__':
    main()